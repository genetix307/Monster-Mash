...
if spawn_zombie_scientist<=0 and instance_number(zombie_scientist)<max_zombie_scientist {instance_create_depth(x,y,depth,zombie_scientist) spawn_zombie_scientist=110}
...
if hud.run_minutes>=7 {spawn_winged_stinger-=1}
...
if hud.run_minutes>=0 and hud.run_seconds<40 {spawn_monster_slime-=1}
...
if hud.run_minutes>=1 and hud.run_seconds>30 {spawn_monster_prototype-=1}
...
if spawn_boss_brain<=0 and instance_number(boss_brain)<max_boss_brain {repeat 3 instance_create_depth(random_range(x-24,x+24),y,depth,boss_brain) spawn_boss_brain=64000 ring_mummy_ground=6}
if hud.run_minutes>=9 {spawn_boss_skullbat-=1}
