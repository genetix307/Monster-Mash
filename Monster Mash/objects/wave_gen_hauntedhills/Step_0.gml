...
if hud.run_seconds>55 {spawn_zombie_scientist-=1}
if spawn_zombie_scientist<=0 and instance_number(zombie_scientist)<max_zombie_scientist {instance_create_depth(x,y,depth,zombie_scientist) spawn_zombie_scientist=80}
...
if spawn_zombie_suit<=0 and instance_number(zombie_suit)<max_zombie_suit {instance_create_depth(x,y,depth,zombie_suit) spawn_zombie_suit=40}

if hud.run_minutes>=5 {spawn_spirit_ghost-=1}
if spawn_spirit_ghost<=0 and hud.run_seconds <=35 and instance_number(spirit_ghost)<max_spirit_ghost {instance_create_depth(x,y,depth,spirit_ghost) spawn_spirit_ghost=130}

if hud.run_seconds>=55 and hud.run_minutes>1 {spawn_mummy_ground-=1}
...
if hud.run_minutes>=7 {spawn_zombie_gusher-=1}
...
if spawn_flying_tentacles<=0 and instance_number(flying_tentacles)<max_flying_tentacles {instance_create_depth(x,y,depth,flying_tentacles) spawn_flying_tentacles=1000}
...
if hud.run_minutes>=3 {spawn_boss_ufo-=1}
...
if hud.run_minutes>=5 {spawn_boss_boogie-=1}
...
if hud.run_minutes>=8 {spawn_boss_brain-=1}
...
if hud.run_minutes>=10 {spawn_boss_skullbat-=1}
if spawn_boss_skullbat<=0 and instance_number(boss_skullbat)<max_boss_skullbat {repeat 4 instance_create_depth(random_range(x-24,x+24),y,depth,boss_skullbat) spawn_boss_skullbat=64000 ring_mummy_ground=38}
if hud.run_minutes=12 {spawn_boss_baby-=1}
