...
//Spawn Enemies
if hud.run_minutes>=0 and hud.run_seconds<5 {spawn_zombie_suit-=1}
if spawn_zombie_suit<=0 and instance_number(zombie_suit)<max_zombie_suit {instance_create_depth(x,y,depth,zombie_suit) spawn_zombie_suit=90}

if hud.run_minutes>=0 {spawn_spirit_ghost-=1}
if spawn_spirit_ghost<=0 and hud.run_seconds <=35 and instance_number(spirit_ghost)<max_spirit_ghost {instance_create_depth(x,y,depth,spirit_ghost) spawn_spirit_ghost=600}
...
if spawn_mummy_ground<=0 and instance_number(mummy_ground)<max_mummy_ground {instance_create_depth(x,y,depth,mummy_ground) spawn_mummy_ground=480}
...
if spawn_vampire_vlad<=0 and instance_number(vampire_vlad)<max_vampire_vlad {instance_create_depth(x,y,depth,vampire_vlad) spawn_vampire_vlad=390}
...
if hud.run_minutes>=6 and hud.run_seconds>20 {spawn_monster_prototype-=1}
...
if spawn_skeleton_bonejamin<=0 and instance_number(skeleton_bonejamin)<max_skeleton_bonejamin {instance_create_depth(x,y,depth,skeleton_bonejamin) spawn_skeleton_bonejamin=110}
...
if hud.run_minutes>=11 {spawn_wave_beast_spider-=1}
...
if hud.run_minutes>=11 {spawn_boss_boogie-=1}
...
if hud.run_minutes>=3 {spawn_boss_brain-=1}
if spawn_boss_brain<=0 and instance_number(boss_brain)<max_boss_brain {repeat 2 instance_create_depth(random_range(x-24,x+24),y,depth,boss_brain) spawn_boss_brain=64000 ring_mummy_ground=38}
if hud.run_minutes>=7 {spawn_boss_skullbat-=1}
if spawn_boss_skullbat<=0 and instance_number(boss_skullbat)<max_boss_skullbat {repeat 2 instance_create_depth(random_range(x-24,x+24),y,depth,boss_skullbat) spawn_boss_skullbat=64000 ring_mummy_ground=38}
